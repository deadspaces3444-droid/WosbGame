...
/* ════════════════════════════════════════════════════════════════════════
   MODULE 01 — CONFIG
   Все константы проекта. Меняешь только здесь.
   ════════════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════════════
   MODULE 02 — STATE
   Единый объект общего состояния. Доступен всем модулям через `S.xxx`.
   ════════════════════════════════════════════════════════════════════════ */
const S = {
    // Кэши загрузки
    gamesCache: {}, clansCache: {}, alliancesCache: {},
    settingsCache: null, faqCache: [], partnersCache: [], tacticsCache: [],
    tradesCache: [], shipsCache: [], siteAdminsCache: [],
    resourcesCache: [], recipesCache: [], discountsCache: [],
    factionsCache: [], portsCache: [], ranksCache: [],
    ...
    // Логотипы (временные)
    partnerLogoData: null, clanLogoData: null, newClanLogoData: null,

    // Текущий контекст
    currentGame: null, currentClan: null,
    currentClanIsAdmin: false, currentClanPass: null,
    pendingClanId: null, currentTab: 'enemies',

    // Модалки / редактирование
    movingItem: null, editingItem: null, editingBuild: null, editingGame: null,
    duplicatingBuild: null, editingTactic: null, editingAlliance: null,
    acceptingTrade: null, editingTradeId: null, editingResourceId: null,

    // Торговля
    tradeFormType: 'buy', tradeFilterType: 'all',
    tradeFilterCat: 'all', tradeFilterClan: 'all',
    tradeSort: 'new', tradeStatusFilter: 'active', tradeOnlyShips: false,

    // Корабли
    shipsFilterLevel: 'all', shipsFilterType: 'all', shipsSort: 'level-desc',

    // Таймеры / каналы
    heartbeatTimer: null, chatChannel: null, onlineChannel: null, notifChannel: null,

    // Чат
    chatMessages: [], notifications: [],
    chatMode: 'guild', chatPrivateWith: null,
    voiceActive: false, voiceRoomOverride: null, jitsiApi: null,

    // Ресурсы
    pricingSaveTimers: {},

    // Карта
    mapSettings: null, currentMapView: 'detailed',
    mapFsZoom: 1, mapFsX: 0, mapFsY: 0,
    mapFsDragging: false, mapFsDragStart: null,
    mapDetailedData: null, mapCleanData: null,

    // Калькулятор
    builderHomeCtrl: null, builderClanCtrl: null
};

/* ════════════════════════════════════════════════════════════════════════
   MODULE 03 — DOM HELPERS
   ════════════════════════════════════════════════════════════════════════ */
...
function setHidden(id, hidden) {
    const el = $(id); if (el) el.hidden = hidden;
}
...
/* ════════════════════════════════════════════════════════════════════════
   MODULE 04 — UTILS
   Чистые функции. Ничего не знают про State и DOM.
   ════════════════════════════════════════════════════════════════════════ */
...
const getMyClanId = () => localStorage.getItem(MY_CLAN_KEY) || null;
const getViewerNick = () => (localStorage.getItem(VIEWER_NICK_KEY) || '').trim();
const isUnlocked = () => S.isAdmin || localStorage.getItem(UNLOCK_KEY) === '1';
const getLeaderClanId = () => S.myAdminClanId || getMyClanId() || S.currentClan || null;
...
async function sendDiscordWebhook(payload) {
    try {
        const webhookUrl = S.settingsCache?.discord_webhook;
...
const currentBgKey = () => S.currentClan ? S.currentClan : 'main';
const currentBgFallback = () => S.currentClan && S.clansCache[S.currentClan]?.bg ? S.clansCache[S.currentClan].bg : null;
function applyBg() {
...
/* ════════════════════════════════════════════════════════════════════════
   MODULE 07 — AUTH
   Права доступа, сайт-админы, вход в админ-панель.
   ════════════════════════════════════════════════════════════════════════ */
function canEditClan(clanId) {
    if (S.isOwner) return true;
    if (S.isAdmin) {
        if (S.myAdminClanId) return clanId === S.myAdminClanId;
        if (S.isMod) return false;
        return true;
    }
    if (clanId === S.currentClan && S.currentClanIsAdmin) return true;
    return false;
}
function isClanLeader() {
    if (S.isOwner && getLeaderClanId()) return true;
    if ((S.isAdmin || S.isMod) && S.myAdminClanId) return true;
    return !!(S.currentClanIsAdmin && getMyClanId());
}
function canAccessClan(clanId) {
    if (S.isOwner) return true;
    if (S.isAdmin && S.myAdminClanId) {
        if (clanId === S.myAdminClanId) return true;
        const my = S.clansCache[S.myAdminClanId], target = S.clansCache[clanId];
        if (!my || !target || !my.alliance_id) return false;
        return target.alliance_id === my.alliance_id;
    }
    if (S.isAdmin && !S.myAdminClanId && !S.isMod) return true;
    if (S.isMod && !S.myAdminClanId) return false;
    const myClan = getMyClanId();
    if (!myClan) return false;
    if (clanId === myClan) return true;
    const my = S.clansCache[myClan], target = S.clansCache[clanId];
    if (!my || !target) return false;
    if (!my.alliance_id) return false;
    return target.alliance_id === my.alliance_id;
}
function canEditBindings() {
    const myEmail = (S.currentSession?.user?.email || '').toLowerCase();
...
async function loadSiteAdmins() {
    try {
        ...
        S.siteAdminsCache = data || [];
    } catch (e) {
        console.warn('site_admins load error:', e.message);
        S.siteAdminsCache = [];
    }
    if (!S.siteAdminsCache.length) {
        S.siteAdminsCache = ADMIN_EMAILS_FALLBACK.map(email => ({
...
function recalcIsAdmin() {
    const email = (S.currentSession?.user?.email || '').toLowerCase();
    const me = S.siteAdminsCache.find(r => (r.email || '').toLowerCase() === email);
    S.isAdmin = !!me;
    S.isOwner = me?.role === 'owner';
    S.isMod = me?.role === 'mod';
    S.siteAdminRole = me?.role || null;
    S.myAdminClanId = me?.clan_id || null;
}
async function renderSiteAdminsAdmin() {
    ...
    document.querySelectorAll('.owner-only').forEach(el => { el.hidden = !S.isOwner; });
    if (!S.siteAdminsCache.length) { container.innerHTML = '<div class="empty">Пока нет админов</div>'; return; }

    let clansList = Object.values(S.clansCache);
    if (!clansList.length) {
        try {
            const { data, error } = await supabase.from('clans').select('id, name').order('name');
            if (error) throw error;
            clansList = data || [];
            clansList.forEach(c => { S.clansCache[c.id] = c; });
        } catch (e) {
            try {
    ...
    const clanOptionsHtml = (selectedId) => {
        ...
        return ['<option value="">— Без привязки —</option>']
            .concat(sorted.map(c => `<option value="${escapeHtml(c.id)}" ${c.id === selectedId ? 'selected' : ''}>🏰 ${escapeHtml(c.name)}</option>`)).join('');
    };
    const order = { owner: 0, admin: 1, mod: 2 };
    const list = S.siteAdminsCache.slice().sort((a, b) => (order[a.role] ?? 9) - (order[b.role] ?? 9));
    ...
    list.forEach(item => {
        ...
        const clanHtml = canBind && !isMe
            ...
            : (clanId && S.clansCache[clanId]
                ? `<span class="role-badge admin">🏰 ${escapeHtml(S.clansCache[clanId].name)}</span>`
        ...
        el.innerHTML = `
            ...
            <div class="txt">
                ...
                <div class="admin-row"><span class="admin-email">${escapeHtml(email)}</span>${roleHtml}</div>
                <div class="admin-row"><span class="admin-email-label">Гильдия:</span>${clanHtml}</div>
            </div>
            <div class="actions">
                ${(S.isOwner || isMe) ? `<button class="edit" title="Сменить пароль">🔑</button>` : ''}
                ${(S.isOwner && !isMe) ? `<button class="nick-btn" title="Сменить никнейм">✏️</button>` : ''}
                ${(S.isOwner && !isMe) ? `<button class="delete" title="Удалить">🗑</button>` : ''}
            </div>`;
...
async function addSiteAdmin(email, password, role, nickname) {
    const msg = $('siteAdminsMsg'); msg.textContent = ''; msg.style.color = '';
    ...
    const btn = $('addSiteAdminBtn'); btn.disabled = true; btn.textContent = '⏳ Создание…';
    const { data, error } = await supabase.rpc('create_site_admin', { new_email: cleanEmail, new_password: password, new_role: role, new_nickname: cleanNick });
...
async function deleteSiteAdmin(email) {
    if (!S.isOwner) { alert('Только владелец может удалять админов'); return; }
...
async function setAdminRole(email, newRole) {
    if (!S.isOwner) { alert('Только владелец'); renderSiteAdminsAdmin(); return; }
...
/* ════════════════════════════════════════════════════════════════════════
   MODULE 08 — SCREENS
   Навигация между экранами + карточка описания гильдии + вход в гильдию.
   ════════════════════════════════════════════════════════════════════════ */
...
function openClanInfo(id, opts = {}) {
    const clan = S.clansCache[id]; if (!clan) return;
    S.pendingClanId = id;
    ...
    if (clan.alliance_id && S.alliancesCache[clan.alliance_id]) {
        const ally = S.alliancesCache[clan.alliance_id];
        const members = Object.values(S.clansCache).filter(c => c.alliance_id === ally.id && c.id !== clan.id);
...
function handleClanClick(id) {
    if (S.isOwner) { openClan(id); return; }
    if (S.isMod && !S.myAdminClanId) { openClanInfo(id, { locked: true }); return; }
    if (S.isAdmin && S.myAdminClanId) {
        if (canAccessClan(id)) { openClan(id); return; }
        openClanInfo(id, { locked: true }); return;
    }
    if (S.isAdmin) { openClan(id); return; }
    const myClan = getMyClanId();
...
function openClan(id, isClanAdminLogin = false) {
    const clan = S.clansCache[id]; if (!clan) return;
    if (!S.isOwner) {
        if (S.isAdmin && S.myAdminClanId) { if (!canAccessClan(id)) { openClanInfo(id, { locked: true }); return; } }
        else if (!S.isAdmin) {
    ...
    S.currentClanIsAdmin = S.isOwner
        || (S.isAdmin && !S.myAdminClanId && !S.isMod)
        || (S.isAdmin && S.myAdminClanId === id)
        || isClanAdminLogin
        || (id === myClan && hasAdminPass);
    S.currentClanPass = localStorage.getItem(CLAN_PASS_KEY) || null;
    if (!S.currentClanIsAdmin) S.currentClanPass = null;
...
function updateAllianceBar() {
    ...
    if (!bar || !list) return;
    const myClan = S.clansCache[S.currentClan];
    if (!myClan || !myClan.alliance_id) { bar.hidden = true; return; }
    const ally = S.alliancesCache[myClan.alliance_id];
    const members = Object.values(S.clansCache).filter(c => c.alliance_id === myClan.alliance_id && c.id !== S.currentClan);
...
/* ════════════════════════════════════════════════════════════════════════
   MODULE 09 — ONLINE
   Heartbeat, realtime-каналы, списки онлайна.
   ════════════════════════════════════════════════════════════════════════ */
async function sendHeartbeat() {
    ...
    const clanId = S.currentClan || null;
    try {
...
function startHeartbeat() {
    ...
    S.heartbeatTimer = setInterval(sendHeartbeat, HEARTBEAT_MS);
    setInterval(updateOnlineCount, 20000);
}
function initRealtime() {
    closeRealtime();
    if (!S.currentClan) return;
    S.onlineChannel = supabase.channel('online')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'online_users' }, () => updateOnlineCount())
        .subscribe();
    const nick = getViewerNick();
    if (nick) {
        S.notifChannel = supabase.channel('notif_' + nick)
...
function closeRealtime() {
    if (S.onlineChannel) { supabase.removeChannel(S.onlineChannel); S.onlineChannel = null; }
    if (S.notifChannel)  { supabase.removeChannel(S.notifChannel);  S.notifChannel  = null; }
}
function buildOnlineRow(u, { withClan = false, iconSelf = '⭐' } = {}) {
    const nick = u.nickname || '—';
    const isGuest = nick.startsWith('guest_');
    const isMe = nick.toLowerCase() === getViewerNick().toLowerCase();
    ...
    const clanName = withClan && u.clan_id && S.clansCache[u.clan_id] ? S.clansCache[u.clan_id].name : null;
    const el = document.createElement('div');
    el.className = 'online-item' + (isGuest ? ' guest' : '') + (isMe ? ' me' : '');
    const icon = isGuest ? '👤' : (isMe ? iconSelf : '🟢');
    ...
    el.innerHTML = `
        ...
        <div class="online-info">
            ...
            <div class="online-meta">
                ...
                ${withClan ? `⏱ ${timeLabel}` : `был в сети ${new Date(u.last_seen).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}`}
            </div>
        </div>
...
async function renderAdminOnlineList() {
    ...
    if (!S.isAdmin && !canEditClan(S.currentClan)) return;
    container.innerHTML = '<div class="empty">Загрузка…</div>';
    const threshold = new Date(Date.now() - ONLINE_WINDOW_MS).toISOString();
    ...
    container.innerHTML = '';
    data.forEach(u => container.appendChild(buildOnlineRow(u, { withClan: true, iconSelf: '⭐' })));
}
async function renderClanOnlineList() {
    const container = $('clanOnlineList'); if (!container) return;
    if (!canEditClan(S.currentClan)) { container.innerHTML = '<div class="empty">Нет доступа</div>'; return; }
    container.innerHTML = '<div class="empty">Загрузка…</div>';
    const threshold = new Date(Date.now() - ONLINE_WINDOW_MS).toISOString();
    const { data, error } = await supabase.from('online_users').select('*').gte('last_seen', threshold).order('last_seen', { ascending: false });
    if (error) { container.innerHTML = `<div class="empty">Ошибка: ${error.message}</div>`; return; }
    if (!data?.length) { container.innerHTML = '<div class="empty">Сейчас никого нет</div>'; return; }
    container.innerHTML = '';
    data.forEach(u => container.appendChild(buildOnlineRow(u, { withClan: false, iconSelf: '⭐' })));
}
...
async function loadNotifications() {
    const nick = getViewerNick();
    if (!nick) { S.notifications = []; renderNotifications(); return; }
...
function renderNotifications() {
    ...
    if (!S.notifications.length) { list.innerHTML = '<p class="empty" style="padding:20px;text-align:center;">Уведомлений пока нет</p>'; return; }
...
/* ════════════════════════════════════════════════════════════════════════
   MODULE 11 — CHAT
   Текстовый + голосовой (Jitsi) + realtime.
   ════════════════════════════════════════════════════════════════════════ */
...
function setChatMode(mode) {
    ...
    if (privTab) { if (S.chatPrivateWith) { privTab.hidden = false; privTab.textContent = '✉️ ' + S.chatPrivateWith; } else privTab.hidden = true; }
    ...
    if (clearBtn) clearBtn.hidden = (!S.isAdmin && !canEditClan(S.currentClan)) || mode === 'voice';
    ...
    if (input) {
        if (mode === 'private' && S.chatPrivateWith) input.placeholder = `Личное для ${S.chatPrivateWith}...`;
...
function buildVoiceRoomName() {
    if (S.voiceRoomOverride) return S.voiceRoomOverride;
    if (S.chatMode === 'leaders') return 'wosb_leaders_hall';
    if (!S.currentClan) return 'wosb_common_hall';
    return 'wosb_guild_' + String(S.currentClan).replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
}
...
function startVoiceChat() {
    const container = $('chatVoiceContainer'); if (!container || S.jitsiApi) return;
    ensureJitsiApi(() => {
        container.innerHTML = '';
        S.jitsiApi = new window.JitsiMeetExternalAPI('meet.jit.si', {
        ...
        S.jitsiApi.addEventListener('videoConferenceLeft', () => stopVoiceChat());
        S.voiceActive = true;
    });
}
function stopVoiceChat() {
    if (S.jitsiApi) { try { S.jitsiApi.dispose(); } catch (e) {} S.jitsiApi = null; }
    const c = $('chatVoiceContainer'); if (c) c.innerHTML = '';
    S.voiceActive = false;
}
async function loadChatMessages() {
    const container = $('chatMessages'); if (!container) return;
    if (S.chatMode === 'guild' && !S.currentClan) { container.innerHTML = '<p class="empty" style="text-align:center;padding:20px;">🏰 Зайдите в гильдию.</p>'; S.chatMessages = []; return; }
    if (S.chatMode === 'private' && !S.chatPrivateWith) { container.innerHTML = '<p class="empty" style="text-align:center;padding:20px;">Выберите получателя.</p>'; S.chatMessages = []; return; }
    if (S.chatMode === 'leaders' && !isClanLeader()) { container.innerHTML = '<p class="empty" style="text-align:center;padding:20px;">🎖 Только для глав гильдий.</p>'; S.chatMessages = []; return; }
    container.innerHTML = '<p class="empty" style="text-align:center;">Загрузка…</p>';
    const myNick = getViewerNick();
    let query = supabase.from('chat_messages').select('*');
    if (S.chatMode === 'guild') query = query.eq('clan_id', S.currentClan).is('recipient', null);
    else if (S.chatMode === 'general') query = query.is('clan_id', null).is('recipient', null);
    else if (S.chatMode === 'leaders') query = query.eq('clan_id', LEADERS_ROOM).is('recipient', null);
    else if (S.chatMode === 'private') {
        const other = S.chatPrivateWith, me = myNick || '__no_nick__';
...
function renderChatMessages() {
    const container = $('chatMessages'); if (!container) return;
    if (!S.chatMessages.length) { container.innerHTML = '<p class="empty" style="text-align:center;padding:20px;">Сообщений пока нет.</p>'; return; }
...
async function sendChatMessage() {
    ...
    if (!text) return;
    if (S.chatMode === 'guild' && !S.currentClan) { alert('Зайдите в гильдию.'); return; }
    if (S.chatMode === 'private' && !S.chatPrivateWith) { alert('Выберите получателя'); return; }
    if (S.chatMode === 'leaders' && !isClanLeader()) { alert('Только для глав гильдий'); return; }
    ...
    if (S.chatMode === 'guild') payload.clan_id = S.currentClan;
    else if (S.chatMode === 'general') { payload.clan_id = null; payload.recipient = null; }
    else if (S.chatMode === 'leaders') { payload.clan_id = LEADERS_ROOM; payload.recipient = null; }
    else if (S.chatMode === 'private') { payload.clan_id = null; payload.recipient = S.chatPrivateWith; }
...
async function clearChat() {
    if (!S.isAdmin && !canEditClan(S.currentClan)) return;
    let label;
    if (S.chatMode === 'general') label = 'ОБЩИЙ чат';
    else if (S.chatMode === 'private') label = `личные с «${S.chatPrivateWith}»`;
    else if (S.chatMode === 'leaders') label = 'чат глав гильдий';
    else label = `чат гильдии «${S.clansCache[S.currentClan]?.name || S.currentClan}»`;
    ...
    if (S.chatMode === 'guild') query = query.eq('clan_id', S.currentClan).is('recipient', null);
        ...
        const other = S.chatPrivateWith;
        query = query.or(`and(nickname.eq.${myNick},recipient.eq.${other}),and(nickname.eq.${other},recipient.eq.${myNick})`);
    }
    const { error } = await query;
    if (error) { alert('Ошибка: ' + error.message); return; }
    S.chatMessages = []; renderChatMessages();
}
function openChat(mode, opts = {}) {
    const clearBtn = $('chatClear'); if (clearBtn) clearBtn.hidden = !S.isAdmin && !canEditClan(S.currentClan);
    $('chatPanel').hidden = false;
    if (mode === 'voice') { S.voiceRoomOverride = opts.room || null; setChatMode('voice'); }
    else if (mode === 'private' && S.chatPrivateWith) setChatMode('private');
    else if (mode === 'general') setChatMode('general');
    else if (mode === 'leaders') setChatMode('leaders');
    else if (mode === 'guild') setChatMode('guild');
    else setChatMode(S.currentClan ? 'guild' : 'general');
}
function openPrivateChat(nickname) {
    if (!nickname) return;
    S.chatPrivateWith = nickname; openChat('private');
    if (!$('screen-admin').hidden) showScreen('home');
    const lp = $('screen-leader'); if (lp && !lp.hidden) lp.hidden = true;
}
function closeChat() {
    $('chatPanel').hidden = true;
    if (S.chatChannel) { supabase.removeChannel(S.chatChannel); S.chatChannel = null; }
    stopVoiceChat();
}
function initChatRealtime() {
    if (S.chatChannel) { supabase.removeChannel(S.chatChannel); S.chatChannel = null; }
    if (S.chatMode === 'voice') return;
    const channelName = 'chat_' + S.chatMode + '_' + (S.chatPrivateWith || S.currentClan || 'global');
    S.chatChannel = supabase.channel(channelName)
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_messages' }, payload => {
            const m = payload.new;
            const myNick = getViewerNick();
            if (S.chatMode === 'guild') { if (m.clan_id !== S.currentClan || m.recipient) return; }
            else if (S.chatMode === 'general') { if (m.clan_id !== null || m.recipient) return; }
            else if (S.chatMode === 'leaders') { if (m.clan_id !== LEADERS_ROOM || m.recipient) return; }
            else if (S.chatMode === 'private') {
                if (!S.chatPrivateWith) return;
                const other = S.chatPrivateWith;
                const a = m.nickname === myNick && m.recipient === other;
                const b = m.nickname === other && m.recipient === myNick;
                if (!a && !b) return;
            }
            if (S.chatMessages.some(x => x.id === m.id)) return;
            S.chatMessages.push(m);
            if (S.chatMessages.length > 200) S.chatMessages.shift();
            renderChatMessages();
        }).subscribe();
}
...
async function loadGames() {
    ...
    S.gamesCache = {};
    (data || []).forEach(g => { S.gamesCache[g.id] = g; });
    const saved = localStorage.getItem(GAME_STORAGE_KEY);
    if (saved && S.gamesCache[saved]) S.currentGame = saved;
    else S.currentGame = Object.keys(S.gamesCache)[0] || null;
...
function renderGamesAdmin() {
    ...
    container.innerHTML = '';
    const list = Object.values(S.gamesCache);
...
async function deleteGame(id, name) {
    ...
    if (error) return alert('Ошибка: ' + error.message);
    if (S.currentGame === id) { S.currentGame = null; localStorage.removeItem(GAME_STORAGE_KEY); }
    await loadGames(); await loadClans();
}
function renderNewClanGameSelect() {
    const sel = $('newClanGame'); if (!sel) return;
    const cur = sel.value; sel.innerHTML = '';
    Object.values(S.gamesCache).forEach(g => { const o = document.createElement('option'); o.value = g.id; o.textContent = g.name; sel.appendChild(o); });
    if (cur && S.gamesCache[cur]) sel.value = cur;
    else if (S.currentGame && S.gamesCache[S.currentGame]) sel.value = S.currentGame;
}
function renderClanRequestGameSelect() {
    const sel = $('crGameId'); if (!sel) return;
    sel.innerHTML = '';
    Object.values(S.gamesCache).forEach(g => { const o = document.createElement('option'); o.value = g.id; o.textContent = g.name; sel.appendChild(o); });
    if (S.currentGame && S.gamesCache[S.currentGame]) sel.value = S.currentGame;
}
...
async function loadAlliances() {
    try {
        ...
        if (error) { S.alliancesCache = {}; return; }
        S.alliancesCache = {};
        (data || []).forEach(a => { S.alliancesCache[a.id] = a; });
        renderAlliancesAdmin(); renderAllianceSelects();
    } catch (e) { S.alliancesCache = {}; }
}
    ...
    list.forEach(a => {
        ...
        el.innerHTML = `
            <div class="alliance-header">
                ...
                <div class="alliance-actions"><button class="edit" title="Редактировать">✏️</button><button class="delete" title="Удалить">🗑</button></div>
            </div>
            ...
            <div class="alliance-clans">
                ...
                ${memberClans.length ? memberClans.map(c => `<span class="alliance-clan-chip">🏰 ${escapeHtml(c.name)}</span>`).join('') : '<span style="color:var(--muted)">— никого —</span>'}
            </div>`;
...
/* ════════════════════════════════════════════════════════════════════════
   MODULE 14 — CLANS
   Загрузка, карточки, списки врагов/друзей/нейтралов/личных, участники, контакты.
   ════════════════════════════════════════════════════════════════════════ */
async function loadClans() {
    const source = S.isAdmin ? 'clans' : 'clans_public';
    const { data, error } = await supabase.from(source).select('*');
    if (error) { console.error('loadClans error:', error); return; }
    S.clansCache = {};
    (data || []).forEach(c => { S.clansCache[c.id] = c; });
    ...
    renderSiteAdminsAdmin();
    if (S.currentClan) updateAllianceBar();
}
function getClansForGame(gameId) { return Object.values(S.clansCache).filter(c => (c.game_id || 'wosb') === gameId); }
function renderHomeCards() {
    const grid = $('clanGrid'); if (!grid) return;
    grid.innerHTML = '';
    const list = S.currentGame ? getClansForGame(S.currentGame) : Object.values(S.clansCache);
    ...
    list.forEach(clan => {
        ...
        let accessible = true;
        if (S.isOwner) accessible = true;
        else if (S.isAdmin && S.myAdminClanId) accessible = canAccessClan(clan.id);
        else if (S.isMod && !S.myAdminClanId) accessible = false;
        else if (S.isAdmin) accessible = true;
...
function renderScopeSelects() {
    ...
    ['pvpScope', 'pbScope', 'buildEditScope', 'buildDupScope', 'evScope'].forEach(id => {
        ...
        else if (S.currentClan && (id === 'pvpScope' || id === 'pbScope' || id === 'evScope')) sel.value = S.currentClan;
    });
}
function renderAll() { if (S.currentClan) TABS.forEach(loadList); }
async function loadList(tab) {
    if (!S.currentClan) return;
    const ul = document.querySelector(`[data-list="${tab}"]`); if (!ul) return;
    ul.innerHTML = '<li class="empty">Загрузка…</li>';
    const { data, error } = await supabase.from(tab).select('*').eq('clan', S.currentClan).order('created_at', { ascending: false });
    ul.innerHTML = '';
    ...
    data.forEach(item => {
        ...
        const canEdit = canEditClan(S.currentClan);
...
function openMoveModal(fromTab, id) { S.movingItem = { fromTab, id }; $('moveModal').hidden = false; }
async function deleteItem(tab, id) {
    ...
    if (S.isAdmin) {
        ...
        const { data: resp, error } = await supabase.rpc('clan_admin_action', {
            action: 'delete', target_table: tab, target_clan: S.currentClan,
            entered_password: S.currentClanPass, record_id: id
        });
...
function renderMembers() {
    ...
    const parsed = parseMembersList(S.clansCache[S.currentClan]?.members_list || '');
...
function renderAdmins() {
    ...
    const parsed = parseMembersList(S.clansCache[S.currentClan]?.admin_nicks || '');
    ...
    if (editField) editField.value = S.clansCache[S.currentClan]?.admin_nicks || '';
}
async function saveClanAdminNicks() {
    if (!canEditClan(S.currentClan)) return;
    ...
    try {
        if (S.isAdmin && !S.isMod) {
            const { error } = await supabase.from('clans').update({ admin_nicks: nicks || null }).eq('id', S.currentClan);
            if (error) throw new Error(error.message);
        } else {
            const { data: resp, error } = await supabase.rpc('clan_admin_action', {
                action: 'update', target_table: 'clans', target_clan: S.currentClan,
                entered_password: S.currentClanPass, record_id: S.currentClan, data: { admin_nicks: nicks || null }
            });
            if (error || resp?.error) throw new Error(resp?.error || error.message);
        }
        if (S.clansCache[S.currentClan]) S.clansCache[S.currentClan].admin_nicks = nicks || null;
        renderAdmins();
        if (statusEl) { statusEl.textContent = '✔ Сохранено'; statusEl.style.color = '#6ee7a7'; }
        await logAdminAction('Обновил список адмиралов', S.clansCache[S.currentClan]?.name || S.currentClan);
...
function renderContacts() {
    ...
    container.innerHTML = '';
    let clans = [];
    if (S.isOwner) clans = Object.values(S.clansCache);
    else if (S.isAdmin && S.myAdminClanId) {
        const my = S.clansCache[S.myAdminClanId];
        if (my) clans = Object.values(S.clansCache).filter(c => c.id === S.myAdminClanId || (my.alliance_id && c.alliance_id === my.alliance_id));
    } else if (S.isAdmin) clans = Object.values(S.clansCache);
    else if (S.currentClan && S.clansCache[S.currentClan]?.alliance_id) {
        const ally = S.clansCache[S.currentClan].alliance_id;
        clans = Object.values(S.clansCache).filter(c => c.alliance_id === ally);
    } else if (S.currentClan) clans = [S.clansCache[S.currentClan]].filter(Boolean);
...
/* ════════════════════════════════════════════════════════════════════════
   MODULE 15 — BUILDS
   ПВП/ПБ билды: рендер, добавление, редактирование, дублирование, удаление.
   ════════════════════════════════════════════════════════════════════════ */
async function renderBuilds(type) {
    ...
    const { data, error } = await supabase.from('builds').select('*')
        .eq('type', type).or(`is_shared.eq.true,clan.eq.${S.currentClan}`)
...
function createBuildCard(item) {
    ...
    const scopeBadge = item.is_shared
        ...
        : `<span class="build-scope clan">🏰 ${escapeHtml(S.clansCache[item.clan]?.name || item.clan || '?')}</span>`;
    const rankBadge = (item.type === 'pb' && item.rank) ? `<span class="build-rank">Ранг ${escapeHtml(item.rank)}</span>` : '';
    const canEditThis = canEditClan(item.clan) || S.isAdmin;
...
async function addBuild(type) {
    ...
    if (S.isAdmin) {
        ...
        const { data: resp, error } = await supabase.rpc('clan_admin_action', {
            action: 'insert', target_table: 'builds', target_clan: S.currentClan,
            entered_password: S.currentClanPass, data: { ...data, is_shared: false }
        });
...
function openBuildEdit(item) {
    S.editingBuild = { id: item.id, type: item.type, clan: item.clan };
...
async function deleteBuild(id, type) {
    ...
    if (S.isAdmin) {
        ...
        const { data: resp, error } = await supabase.rpc('clan_admin_action', {
            action: 'delete', target_table: 'builds', target_clan: S.currentClan,
            entered_password: S.currentClanPass, record_id: id
        });
        if (error || resp?.error) return alert('Ошибка: ' + (resp?.error || error.message));
    }
...
async function renderEvents() {
    ...
    const { data, error } = await supabase.from('events').select('*')
        .or(`is_shared.eq.true,clan.eq.${S.currentClan}`).order('event_date', { ascending: true });
    if (error) { container.innerHTML = `<div class="empty">Ошибка: ${error.message}</div>`; return; }
    ...
    data.forEach(ev => {
        ...
        const scopeBadge = ev.is_shared
            ...
            : `<span class="event-badge clan">🏰 ${escapeHtml(S.clansCache[ev.clan]?.name || ev.clan)}</span>`;
        ...
        if (canDel) {
            card.querySelector('.delete').addEventListener('click', async () => {
                ...
                if (S.isAdmin) {
                    ...
                    const { data: resp, error } = await supabase.rpc('clan_admin_action', {
                        action: 'delete', target_table: 'events', target_clan: S.currentClan,
                        entered_password: S.currentClanPass, record_id: ev.id
                    });
                    if (error || resp?.error) return alert('Ошибка: ' + (resp?.error || error.message));
                }
                renderEvents();
...
async function renderTreasury() {
    ...
    container.innerHTML = '<div class="empty">Загрузка…</div>';
    const { data, error } = await supabase.from('treasury').select('*').eq('clan', S.currentClan).order('created_at', { ascending: false });
    if (error) { container.innerHTML = `<div class="empty">Ошибка: ${error.message}</div>`; return; }
    const list = data || [];
    let balance = 0;
    ...
    list.forEach(t => {
        ...
        const canDel = canEditClan(S.currentClan);
        item.innerHTML = `
        ...
        if (canDel) item.querySelector('button').addEventListener('click', async () => {
            ...
            if (S.isAdmin) {
                ...
                const { data: resp, error } = await supabase.rpc('clan_admin_action', {
                    action: 'delete', target_table: 'treasury', target_clan: S.currentClan,
                    entered_password: S.currentClanPass, record_id: t.id
                });
                if (error || resp?.error) return alert('Ошибка: ' + (resp?.error || error.message));
            }
            renderTreasury();
...
/* ════════════════════════════════════════════════════════════════════════
   MODULE 18 — SHIPS
   Справочник кораблей + фильтры + селекты.
   ════════════════════════════════════════════════════════════════════════ */
async function loadShips() {
    const { data, error } = await supabase.from('ships').select('*')
        .order('level', { ascending: false }).order('name');
    if (error) { console.warn('ships load error:', error.message); S.shipsCache = []; }
    else S.shipsCache = data || [];
    renderShipsGrid();
    fillShipSelects();
    renderAdminShips();
}
function renderShipsGrid() {
    ...
    let list = S.shipsCache.filter(s => {
        if (S.shipsFilterLevel !== 'all' && s.level !== parseInt(S.shipsFilterLevel)) return false;
        if (S.shipsFilterType !== 'all' && s.type !== S.shipsFilterType) return false;
...
function fillShipSelects() {
    ['pvpShip', 'pbShip'].forEach(id => {
        ...
        S.shipsCache.slice().sort((a, b) => a.name.localeCompare(b.name)).forEach(s => {
            const o = document.createElement('option');
            o.value = s.name;
    ...
    if (S.builderHomeCtrl) S.builderHomeCtrl.fillTargetShip();
    if (S.builderClanCtrl) S.builderClanCtrl.fillTargetShip();
    fillScShipSelect();
}
function renderAdminShips() {
    const container = $('adminShipsList'); if (!container) return;
    container.innerHTML = '';
    if (!S.shipsCache.length) { container.innerHTML = '<div class="empty">Корабли не загружены. Запустите SQL из README.</div>'; return; }
    S.shipsCache.forEach(s => {
        const el = document.createElement('div');
...
function createShipBuilder(prefix) {
    ...
    const ctx = prefix ? 'clan' : 'home';
    const st = { rows: [], counter: 0 };
    ...
    function addRow() { st.counter++; st.rows.push({ id: st.counter, category: 'resource', name: '', qty: 1 }); }
    function computeAvg(name) {
        const lower = (name || '').trim().toLowerCase();
        if (!lower) return null;
        const res = S.resourcesCache.find(r => (r.name || '').trim().toLowerCase() === lower);
        if (!res) return null;
        return Number(res.price) || 0;
    }
    function updateRowImage(rowId) {
        const row = st.rows.find(x => x.id === rowId); if (!row) return;
    ...
    function renderRows() {
        ...
        st.rows.forEach(row => {
            ...
            tdName.innerHTML = `
                <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;">
                    ...
                    <input type="text" class="builder-name" data-id="${row.id}" list="builderSuggestions_${ctx}" placeholder="Название компонента" value="${escapeHtml(row.name)}" style="flex:1;min-width:120px;">
                </div>`;
        ...
        let dl = document.getElementById('builderSuggestions_' + ctx);
        if (!dl) {
            dl = document.createElement('datalist');
            dl.id = 'builderSuggestions_' + ctx;
            document.body.appendChild(dl);
        }
        const unique = [...new Set(S.resourcesCache.map(r => r.name))].sort();
        dl.innerHTML = unique.map(n => `<option value="${escapeHtml(n)}">`).join('');
        tbody.querySelectorAll('.builder-cat').forEach(el => {
            el.addEventListener('change', e => {
                const r = st.rows.find(x => x.id === parseInt(e.target.dataset.id));
        ...
        st.rows.forEach(r => updateRowImage(r.id));
        recalc();
    }
    function recalc() {
    ...
    function copyResult() {
        if (!st.rows.length) { alert('Список пуст'); return; }
        ...
        navigator.clipboard.writeText(text).then(() => alert('📋 Результат скопирован!'), () => prompt('Скопируйте вручную:', text));
    }
    function bind() {
        on(ids.addRow, 'click', () => { addRow(); renderRows(); });
        on(ids.clear, 'click', () => {
            if (!confirm('Очистить калькулятор?')) return;
            st.rows = []; addRow(); renderRows();
        });
    ...
    function init() { bind(); if (!st.rows.length) addRow(); fillTargetShip(); renderRows(); }
    function refresh() { fillTargetShip(); renderRows(); }
    return { init, refresh, recalc, fillTargetShip };
}
function initShipBuilders() {
    S.builderHomeCtrl = createShipBuilder('');
    S.builderHomeCtrl.init();
    S.builderClanCtrl = createShipBuilder('builder');
    S.builderClanCtrl.init();
}

/* ════════════════════════════════════════════════════════════════════════
   MODULE 20 — RESOURCES
   Справочник ресурсов + авто-цены + пресет.
   ════════════════════════════════════════════════════════════════════════ */
async function loadResourcePrices() {
    const { data, error } = await supabase.from('resource_prices').select('*')
        .order('sort_order', { ascending: true }).order('name', { ascending: true });
    if (error) { console.warn('resource_prices load error:', error.message); S.resourcesCache = []; }
    else S.resourcesCache = data || [];
    renderPricingGrid();
    renderResourcePricesHome();
    if (S.builderHomeCtrl) S.builderHomeCtrl.refresh();
    if (S.builderClanCtrl) S.builderClanCtrl.refresh();
}
function renderPricingGrid() {
    const container = $('pricingGroups'); if (!container) return;
    const q = ($('pricing-search')?.value || '').trim().toLowerCase();
    let list = S.resourcesCache.slice();
    ...
    container.querySelectorAll('.pricing-card-input').forEach(inp => {
        inp.addEventListener('input', e => {
            ...
            clearTimeout(S.pricingSaveTimers[id]);
            S.pricingSaveTimers[id] = setTimeout(() => saveResourcePrice(id, e.target.value), 700);
        });
...
async function saveResourcePrice(id, rawPrice) {
    ...
    const r = S.resourcesCache.find(x => x.id === id);
    if (r) r.price = price;
...
function openResourceEditModal(id) {
    const r = id ? S.resourcesCache.find(x => x.id === id) : null;
    S.editingResourceId = r ? r.id : null;
...
/* ════════════════════════════════════════════════════════════════════════
   MODULE 21 — RECIPES
   Рецепты + скидки + публичный расчёт стоимости постройки.
   ════════════════════════════════════════════════════════════════════════ */
async function loadRecipes() {
    const { data, error } = await supabase.from('ship_recipes').select('*').order('sort_order').order('id');
    if (error) { console.warn('ship_recipes load:', error.message); S.recipesCache = []; }
    else S.recipesCache = data || [];
...
async function loadDiscounts() {
    ...
    if (error) { console.warn('discounts load:', error.message); S.discountsCache = []; }
    else S.discountsCache = data || [];
...
function fillScCities() {
    ...
    sel.innerHTML = '<option value="">— Не указан —</option>' +
        S.portsCache.map(p => `<option value="${escapeHtml(p.name)}">${escapeHtml(p.name)}</option>`).join('');
    if (cur && S.portsCache.some(p => p.name === cur)) sel.value = cur;
}
...
function fillScFactionSelect() {
    ...
    sel.innerHTML = '<option value="">— Не указана —</option>' +
        S.factionsCache.filter(f => f.type !== 'neutral').map(f => `<option value="${escapeHtml(f.name)}">${escapeHtml(f.name)}</option>`).join('');
    if (cur) sel.value = cur;
}
function getActiveDiscounts(city, faction) {
    const today = new Date().toISOString().slice(0, 10);
    return S.discountsCache.filter(d => {
...
function renderShipCostPublic() {
    ...
    const recipe = S.recipesCache.filter(r => r.ship_id === shipName);
    if (!recipe.length) {
    ...
    let baseSum = 0;
    recipe.forEach(r => {
        const res = S.resourcesCache.find(x => x.id === r.resource_id);
...
function renderRecipeRows() {
    ...
    const rows = S.recipesCache.filter(r => r.ship_id === ship).sort((a, b) => (a.sort_order||0) - (b.sort_order||0));
    container.innerHTML = '';
...
function addRecipeRowUI(resourceId = '', qty = 1, id = null) {
    ...
    row.innerHTML = `
        ...
        <select class="rr-res">
            ...
            ${S.resourcesCache.map(r => `<option value="${escapeHtml(r.id)}" ${r.id === resourceId ? 'selected' : ''}>${escapeHtml(r.name)}</option>`).join('')}
        </select>
    ...
    const updateImg = () => {
        const r = S.resourcesCache.find(x => x.id === sel.value);
...
function renderDiscountsAdmin() {
    ...
    container.innerHTML = '';
    if (!S.discountsCache.length) { container.innerHTML = '<div class="empty">Скидок пока нет</div>'; return; }
    S.discountsCache.forEach(d => {
...
function fillDiscountFactionSelect() {
    ...
    sel.innerHTML = '<option value="">— Не выбрана —</option>' +
        S.factionsCache.map(f => `<option value="${escapeHtml(f.name)}">${escapeHtml(f.name)}</option>`).join('');
    if (cur) sel.value = cur;
}
...
async function loadMapSettings() {
    ...
    if (error) { console.warn('map_settings load:', error.message); S.mapSettings = null; }
    else S.mapSettings = data;
    renderMapPreview(); renderMapAdmin();
}
function getMapUrls() {
    const s = S.mapSettings || {};
    return {
...
function renderMapPreview() {
    ...
    document.querySelectorAll('.map-view-btn').forEach(b => b.classList.toggle('active', b.dataset.view === S.currentMapView));
    const src = S.currentMapView === 'clean' ? urls.clean : urls.detailed;
    img.src = src || '';
    img.onerror = () => {
        if (S.currentMapView === 'detailed' && urls.clean) { img.src = urls.clean; }
        else if (S.currentMapView === 'clean' && urls.detailed) { img.src = urls.detailed; }
...
function openMapFullscreen() {
    ...
    img.src = src;
    img.onerror = () => { img.src = S.currentMapView === 'clean' ? urls.detailed : urls.clean; };
    const titleEl = $('map-fs-title'); if (titleEl) titleEl.textContent = urls.title;
    S.mapFsZoom = 1; S.mapFsX = 0; S.mapFsY = 0; applyMapFsTransform();
    fs.hidden = false;
...
function applyMapFsTransform() {
    ...
    img.style.transform = `translate(${S.mapFsX}px, ${S.mapFsY}px) scale(${S.mapFsZoom})`;
}
function setMapFsZoom(z) {
    S.mapFsZoom = Math.max(0.5, Math.min(6, z));
    if (S.mapFsZoom <= 1) { S.mapFsX = 0; S.mapFsY = 0; }
    applyMapFsTransform();
}
function resetMapFs() { S.mapFsZoom = 1; S.mapFsX = 0; S.mapFsY = 0; applyMapFsTransform(); }
function renderMapAdmin() {
    ...
    S.mapDetailedData = null; S.mapCleanData = null;
    const msg = $('map-admin-msg'); if (msg) msg.textContent = '';
}

/* ════════════════════════════════════════════════════════════════════════
   MODULE 23 — FACTIONS / PORTS / RANKS
   ════════════════════════════════════════════════════════════════════════ */
async function loadFactions() {
    const { data, error } = await supabase.from('factions').select('*').order('sort_order');
    if (error) { S.factionsCache = []; return; }
    S.factionsCache = data || [];
    renderFactionsAdmin();
...
function renderFactionsAdmin() {
    ...
    container.innerHTML = '';
    if (!S.factionsCache.length) { container.innerHTML = '<div class="empty">Нет фракций</div>'; return; }
    const typeLabels = { military: '⚔️ Военная', trade: '💰 Торговая', pirate: '☠️ Пиратская', neutral: '🕊 Нейтральная' };
    S.factionsCache.forEach(f => {
        const el = document.createElement('div');
        el.className = 'partners-admin-item';
        el.innerHTML = `
...
function renderPortsAdmin() {
    ...
    container.innerHTML = '';
    if (!S.portsCache.length) { container.innerHTML = '<div class="empty">Нет портов</div>'; return; }
    const typeLabels = { city: '🏙 Город', port: '⚓ Порт', neutral_bay: '🕊 Нейтральная', pirate_bay: '☠️ Пиратская' };
    S.portsCache.forEach(p => {
        const faction = S.factionsCache.find(f => f.id === p.faction_id);
        const el = document.createElement('div');
        el.className = 'partners-admin-item';
        el.innerHTML = `
...
function renderRanksAdmin() {
    ...
    container.innerHTML = '';
    if (!S.ranksCache.length) { container.innerHTML = '<div class="empty">Нет данных о рангах</div>'; return; }
    S.ranksCache.forEach(r => {
        const el = document.createElement('div');
        el.className = 'partners-admin-item';
        el.innerHTML = `
...
function fillPortFactionSelect() {
    ...
    sel.innerHTML = '<option value="">— Без фракции —</option>' +
        S.factionsCache.map(f => `<option value="${escapeHtml(f.id)}">${escapeHtml(f.name)}</option>`).join('');
    if (cur) sel.value = cur;
}
function openPortEditModal(port) {
    ...
    sel.innerHTML = '<option value="">— Без фракции —</option>' +
        S.factionsCache.map(f => `<option value="${escapeHtml(f.id)}" ${f.id === port.faction_id ? 'selected' : ''}>${escapeHtml(f.name)}</option>`).join('');
    $('pe-msg').textContent = '';
...
function renderTradeClanSelect() {
    ...
    const list = S.currentGame ? getClansForGame(S.currentGame) : Object.values(S.clansCache);
    list.forEach(c => { const o = document.createElement('option'); o.value = c.id; o.textContent = c.name; sel.appendChild(o); });
    if (cur && S.clansCache[cur]) sel.value = cur;
}
function renderTradeClanFilters() {
    ...
    wrap.innerHTML = `<span class="tm-chip active" data-clan="all">Все гильдии</span>` +
        Object.values(S.clansCache).map(c => `<span class="tm-chip" data-clan="${c.id}">🏰 ${escapeHtml(c.name)}</span>`).join('');
    if (S.tradeFilterClan !== 'all' && !S.clansCache[S.tradeFilterClan]) S.tradeFilterClan = 'all';
    wrap.querySelectorAll('.tm-chip').forEach(chip => {
        if (chip.dataset.clan === S.tradeFilterClan) chip.classList.add('active');
...
async function renderTrades() {
    ...
    if (error) { container.innerHTML = `<div class="empty">Ошибка: ${error.message}</div>`; return; }
    S.tradesCache = data || [];
    renderTradeCounters(); renderTradeListings(); renderResourcePricesHome();
}
function renderTradeCounters() {
    let buyGold = 0, sellGold = 0, buyN = 0, sellN = 0;
    S.tradesCache.forEach(t => {
...
function renderTradeMyCounter() {
    ...
    const count = S.tradesCache.filter(t => (t.nickname || '').toLowerCase() === myNick && t.status !== 'done').length;
...
function tradeVisibleListings() {
    ...
    const myNick = getViewerNick().toLowerCase();
    let list = S.tradesCache.filter(t => {
        if (S.tradeStatusFilter === 'active' && t.status === 'done') return false;
        if (S.tradeStatusFilter === 'done' && t.status !== 'done') return false;
        if (S.tradeFilterType !== 'all' && t.type !== S.tradeFilterType) return false;
        if (S.tradeFilterCat !== 'all' && t.category !== S.tradeFilterCat) return false;
        if (S.tradeOnlyShips && t.category !== 'ship') return false;
        if (S.tradeFilterClan !== 'all' && t.clan !== S.tradeFilterClan) return false;
        if (q) {
...
function renderTradeListings() {
    ...
    items.forEach(t => {
        ...
        const canDelete = S.isAdmin || (isMine && !isAccepted && !isDone);
        const canEdit = isMine && !isAccepted && !isDone;
        const canRepeat = isDone && isMine;
        const typeLabel = t.type === 'buy' ? '🛒 Куплю' : '💰 Продам';
        const clanName = S.clansCache[t.clan]?.name || t.clan;
        const authorCompleted = S.tradesCache.filter(x =>
            x.status === 'done' &&
        ...
        let shipThumbHtml = '';
        if (t.category === 'ship') {
            const shipObj = S.shipsCache.find(s => s.name.toLowerCase() === (t.name || '').toLowerCase());
            ...
            const resObj = S.resourcesCache.find(r => (r.name || '').trim().toLowerCase() === (t.name || '').trim().toLowerCase());
...
function updateShipPickerVisibility() {
    ...
    const picker = $('tm-ship-picker'); if (!picker) return;
...
function repeatTrade(t) {
    if (!t) return;
    const clanSel = $('tm-clan'); if (clanSel && S.clansCache[t.clan]) clanSel.value = t.clan;
    const catSel = $('tm-category'); if (catSel) catSel.value = t.category;
    S.tradeFormType = t.type;
...
function closeAcceptTradeModal() { $('acceptTradeModal').hidden = true; S.acceptingTrade = null; }

/* ════════════════════════════════════════════════════════════════════════
   MODULE 25 — CONTENT
   FAQ + партнёры + тактика + новости VK.
   ════════════════════════════════════════════════════════════════════════ */
...
function renderFaq() {
    ...
    container.innerHTML = '';
    if (!S.faqCache.length) { container.innerHTML = '<div class="empty">Пока нет вопросов</div>'; return; }
    S.faqCache.forEach(item => {
...
function renderFaqAdmin() {
    ...
    container.innerHTML = '';
    if (!S.faqCache.length) { container.innerHTML = '<div class="empty">Пока нет</div>'; return; }
    S.faqCache.forEach(item => {
        const el = document.createElement('div');
...
function renderPartnersHome() {
    ...
    list.innerHTML = '';
    if (!S.partnersCache.length) { section.hidden = true; return; }
    section.hidden = false;
    S.partnersCache.forEach(p => { list.appendChild(isYouTubeUrl(p.url) ? createYouTubeCard(p) : createPartnerCard(p)); });
}
...
function renderPartnersAdmin() {
    ...
    container.innerHTML = '';
    if (!S.partnersCache.length) { container.innerHTML = '<div class="empty">Пока нет партнёров</div>'; return; }
    S.partnersCache.forEach((p, idx) => {
        const el = document.createElement('div');
        el.className = 'partners-admin-item';
        ...
        el.innerHTML = `
            ...
            <div class="actions">
                ...
                <button class="down" ${idx === S.partnersCache.length - 1 ? 'disabled style="opacity:.3"' : ''}>▼</button>
...
async function movePartner(id, dir) {
    const idx = S.partnersCache.findIndex(p => p.id === id); if (idx === -1) return;
    const swapIdx = idx + dir; if (swapIdx < 0 || swapIdx >= S.partnersCache.length) return;
    const a = S.partnersCache[idx], b = S.partnersCache[swapIdx];
...
function renderTacticsAdmin() {
    ...
    container.innerHTML = '';
    if (!S.tacticsCache.length) { container.innerHTML = '<div class="empty">Пока нет разделов</div>'; return; }
    S.tacticsCache.forEach((t, idx) => {
        const el = document.createElement('div');
        el.className = 'partners-admin-item';
        el.innerHTML = `
            ...
            <div class="actions">
                <button class="up" ${idx === 0 ? 'disabled style="opacity:.3"' : ''}>▲</button>
                <button class="down" ${idx === S.tacticsCache.length - 1 ? 'disabled style="opacity:.3"' : ''}>▼</button>
...
async function moveTactic(id, dir) {
    const idx = S.tacticsCache.findIndex(t => t.id === id); if (idx === -1) return;
    const swapIdx = idx + dir; if (swapIdx < 0 || swapIdx >= S.tacticsCache.length) return;
    const a = S.tacticsCache[idx], b = S.tacticsCache[swapIdx];
...
async function loadNews() {
    ...
    const sourceUrl = S.settingsCache?.news_rss_url || `https://vk.com/@${VK_DOMAIN}`;
...
/* ════════════════════════════════════════════════════════════════════════
   MODULE 26 — CLAN REQUESTS
   Заявки на создание гильдий.
   ════════════════════════════════════════════════════════════════════════ */
...
async function renderClanRequestsAdmin() {
    ...
    if (!S.isOwner) { container.innerHTML = '<div class="empty">Доступно только владельцу</div>'; return; }
    container.innerHTML = '<div class="empty">Загрузка…</div>';
    ...
    data.forEach(r => {
        ...
        card.innerHTML = `
            ...
            <div class="application-grid">
                ...
                <div><b>Игра</b>${escapeHtml(S.gamesCache[r.game_id]?.name || r.game_id || '—')}</div>
...
function openLeaderPanel() {
    ...
    if (nameEl) nameEl.textContent = S.clansCache[clanId]?.name || clanId;
    showScreen('leader');
...
async function renderLeaderAlliances(clanId) {
    ...
    if (incCont) {
        ...
        if (!incoming?.length) incCont.innerHTML = '<div class="empty">Входящих заявок нет</div>';
            ...
            const fromClan = S.clansCache[r.from_clan]?.name || r.from_clan;
            card.innerHTML = `
                <div class="application-head">
    ...
    if (outCont) {
        ...
        if (!outgoing?.length) outCont.innerHTML = '<div class="empty">Исходящих заявок нет</div>';
            ...
            const toClan = S.clansCache[r.to_clan]?.name || r.to_clan;
            card.innerHTML = `
                <div class="application-head">
...
async function respondAllianceRequest(id, status, clanId) {
    ...
    if (status === 'accepted') {
        let allyId = S.clansCache[req.from_clan]?.alliance_id || S.clansCache[req.to_clan]?.alliance_id;
        if (!allyId) {
            allyId = 'union_' + Date.now().toString(36);
            await supabase.from('alliances').insert({
                id: allyId,
                name: `${S.clansCache[req.from_clan]?.name || req.from_clan} & ${S.clansCache[req.to_clan]?.name || req.to_clan}`,
    ...
    await createNotification(req.from_clan, 'system',
        ...
        `Гильдия ${S.clansCache[clanId]?.name || clanId} ${status === 'accepted' ? 'приняла союз' : 'отклонила союз'}`,
        null);
    renderLeaderAlliances(clanId);
}
...
/* ════════════════════════════════════════════════════════════════════════
   MODULE 28 — ADMIN
   Рендеры админки: селект гильдий, поля, статистика, настройки.
   ════════════════════════════════════════════════════════════════════════ */
function renderAdminClanSelect() {
    const sel = $('adminClanSelect'); if (!sel) return;
    const cur = sel.value; sel.innerHTML = '';
    Object.values(S.clansCache).forEach(c => { const o = document.createElement('option'); o.value = c.id; o.textContent = c.name; sel.appendChild(o); });
    if (cur && S.clansCache[cur]) sel.value = cur;
    updateAdminFields();
}
...
function applyAdminUI() {
    ...
    const canAccessPanel = S.isAdmin && !S.isMod;
    setHidden('adminLoginBtn', S.isAdmin);
    setHidden('adminLogoutBtn', !S.isAdmin);
    setHidden('adminPanelBtn', !canAccessPanel);
    setHidden('adminLogoutBtn2', !S.isAdmin);
    setHidden('adminPanelBtn2', !canAccessPanel);
    setHidden('adminPanelBtn3', !canAccessPanel);
    const label = S.isAdmin ? (S.isOwner ? '👑 Владелец' : S.siteAdminRole === 'mod' ? '🎖 Глава Клана' : '⚙️ Админ') : '';
    ['adminInfo','adminInfo2','adminInfo3'].forEach(id => { const el = $(id); if (el) el.textContent = label; });
    document.querySelectorAll('.admin-only').forEach(el => { el.hidden = !S.isAdmin; if (!S.isAdmin) el.style.display = ''; });
    document.querySelectorAll('.add-form.admin-only').forEach(el => { el.style.display = S.isAdmin ? 'flex' : 'none'; });
    document.querySelectorAll('.clan-admin-only').forEach(el => {
        const show = canEditClan(S.currentClan);
        el.hidden = !show; if (!show) el.style.display = '';
    });
    document.querySelectorAll('.add-form.clan-admin-only').forEach(el => {
        el.style.display = canEditClan(S.currentClan) ? 'flex' : 'none';
    });
    document.querySelectorAll('.owner-only').forEach(el => { el.hidden = !S.isOwner; });
    const clearBtn = $('chatClear'); if (clearBtn) clearBtn.hidden = !S.isAdmin;
    renderAll();
    updateLeaderButtonsVisibility();
}
function openAdminPage() {
    if (!S.isAdmin || S.isMod) return;
...
/* ════════════════════════════════════════════════════════════════════════
   MODULE 29 — BINDINGS
   Единая регистрация всех обработчиков по модулям.
   ════════════════════════════════════════════════════════════════════════ */
function bindTheme() {
    ['themeToggle','themeToggle2','themeToggle3'].forEach(id => on(id, 'click', toggleTheme));
    on('themeToggle4', 'click', toggleTheme);

    on('bgChangeBtn', 'click', () => { if (!canEditClan(S.currentClan)) return; bgFileInput.value = ''; bgFileInput.click(); });
...
function bindAuth() {
    ...
    on('doAdminLogin', 'click', async () => {
        ...
        await loadSiteAdmins();
        if (!S.isAdmin) { err.textContent = 'Этот email не в списке админов сайта'; await supabase.auth.signOut(); return; }
    ...
    supabase.auth.onAuthStateChange(async (_e, session) => {
        ...
        await loadSiteAdmins(); await loadClans(); applyAdminUI();
    });
    on('addSiteAdminBtn', 'click', () => addSiteAdmin(val('newSiteAdminEmail'), val('newSiteAdminPassword'), val('newSiteAdminRole'), val('newSiteAdminNickname')));
...
function bindScreens() {
    on('backToHomeBtn', 'click', () => { S.pendingClanId = null; showScreen('home'); });
    on('clanViewBtn', 'click', () => { if (S.pendingClanId) openClan(S.pendingClanId); });
    on('clanLoginBtn', 'click', () => {
        if (!S.pendingClanId) return;
        const clan = S.clansCache[S.pendingClanId]; if (!clan) return;
    ...
    on('doClanLogin', 'click', async () => {
        ...
        const { data: ok, error: rpcErr } = await supabase.rpc('verify_clan_password', { clan_id: S.pendingClanId, entered_password: entered });
        let isClanAdminLogin = false;
        if (rpcErr || !ok) {
            const { data: okAdmin, error: admErr } = await supabase.rpc('verify_clan_admin_password', { cid: S.pendingClanId, entered });
        ...
        localStorage.setItem(MY_CLAN_KEY, S.pendingClanId);
        $('clanPassModal').hidden = true;
        await logView(nick, S.pendingClanId, isClanAdminLogin ? 'вход (адмирал)' : 'вход');
        sendHeartbeat();
        const tn = $('tm-nickname'); if (tn && !tn.value) tn.value = nick;
        const cid = S.pendingClanId; S.pendingClanId = null;
    ...
    on('backBtn', 'click', () => {
        S.currentClan = null; S.currentClanIsAdmin = false; S.currentClanPass = null;
    ...
    on('clanLeaveBtn', 'click', () => {
        ...
        [UNLOCK_KEY, LAST_CLAN_KEY, CLAN_PASS_KEY, CLAN_ADMIN_PASS_KEY, MY_CLAN_KEY].forEach(k => localStorage.removeItem(k));
        S.currentClan = null; S.currentClanIsAdmin = false; S.currentClanPass = null;
    ...
    document.querySelectorAll('.side-item').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            else if (section === 'builder') { if (S.builderClanCtrl) S.builderClanCtrl.refresh(); }
...
function bindCatalogs() {
    on('gameAddBtn', 'click', async () => {
        ...
        if (S.gamesCache[id]) { flashStatusEl(statusEl, 'ID существует', '#ff7a7a'); return; }
        const { error } = await supabase.from('games').insert({ id, name, image: image || null, bg: bg || null, sort_order: Object.keys(S.gamesCache).length });
        if (error) { flashStatusEl(statusEl, 'Ошибка: ' + error.message, '#ff7a7a'); return; }
        ['gameId','gameName','gameImage','gameBg'].forEach(i => { const el = $(i); if (el) el.value = ''; });
        flashStatusEl(statusEl, '✔ Добавлено', '#6ee7a7');
        await loadGames();
    });
    on('cancelGameEdit', 'click', () => { $('gameEditModal').hidden = true; S.editingGame = null; });
    on('saveGameEdit', 'click', async () => {
        if (!S.editingGame) return;
        ...
        const { error } = await supabase.from('games').update({ name, image: image || null, bg: bg || null }).eq('id', S.editingGame.id);
        if (error) { msg.textContent = 'Ошибка: ' + error.message; msg.style.color = '#ff7a7a'; return; }
        $('gameEditModal').hidden = true; S.editingGame = null; await loadGames();
    });
    on('allyAddBtn', 'click', async () => {
        ...
        if (S.alliancesCache[id]) { msg.textContent = 'ID занят'; msg.style.color = '#ff7a7a'; return; }
    ...
    on('cancelAllyEdit', 'click', () => { $('allyEditModal').hidden = true; S.editingAlliance = null; });
    on('saveAllyEdit', 'click', async () => {
        if (!S.editingAlliance) return;
        const name = val('allyEditName').trim(), desc = val('allyEditDesc').trim();
        const msg = $('allyEditMsg');
        if (!name) { msg.textContent = 'Укажи название'; msg.style.color = '#ff7a7a'; return; }
        const { error } = await supabase.from('alliances').update({ name, description: desc || null }).eq('id', S.editingAlliance.id);
        if (error) { msg.textContent = 'Ошибка: ' + error.message; msg.style.color = '#ff7a7a'; return; }
        $('allyEditModal').hidden = true; S.editingAlliance = null; await loadAlliances();
    });
}
function bindClans() {
    document.querySelectorAll('.tab').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const tabContent = $('tab-' + S.currentTab);
    ...
    on('addBtn', 'click', async () => {
        ...
        if (S.isAdmin) {
            const { error } = await supabase.from(S.currentTab).insert({ ...data, clan: S.currentClan });
            if (error) { flashStatus('Ошибка: ' + error.message, '#ff7a7a'); return; }
        } else {
            const { data: resp, error } = await supabase.rpc('clan_admin_action', {
                action: 'insert', target_table: S.currentTab, target_clan: S.currentClan,
                entered_password: S.currentClanPass, data
            });
    ...
    on('saveEdit', 'click', async () => {
        ...
        if (S.isAdmin) {
            ...
            const { data: resp, error } = await supabase.rpc('clan_admin_action', {
                action: 'update', target_table: tab, target_clan: S.currentClan,
                entered_password: S.currentClanPass, record_id: id, data
            });
            if (error || resp?.error) { $('editError').textContent = 'Ошибка: ' + (resp?.error || error.message); return; }
        }
        $('editModal').hidden = true; S.editingItem = null; loadList(tab);
    });
    ['editPlayerGuild','editNickname','editFaction','editNote'].forEach(id => {
        on(id, 'keydown', e => { if (e.key === 'Enter') $('saveEdit').click(); });
    });
    on('cancelMove', 'click', () => { $('moveModal').hidden = true; S.movingItem = null; });
    document.querySelectorAll('#moveModal [data-target]').forEach(btn => {
        btn.addEventListener('click', async () => {
            if (!S.movingItem || !S.currentClan) return;
            const { fromTab, id } = S.movingItem;
            const toTab = btn.dataset.target;
            $('moveModal').hidden = true; S.movingItem = null;
            ...
            if (S.isAdmin) {
                const { error: insErr } = await supabase.from(toTab).insert({
                    ...
                    faction: data.faction, note: data.note, clan: S.currentClan
                });
                if (insErr) return alert(insErr.message);
                await supabase.from(fromTab).delete().eq('id', id);
            } else {
                const { data: resp1, error: err1 } = await supabase.rpc('clan_admin_action', {
                    action: 'insert', target_table: toTab, target_clan: S.currentClan,
                    entered_password: S.currentClanPass,
                ...
                const { data: resp2, error: err2 } = await supabase.rpc('clan_admin_action', {
                    action: 'delete', target_table: fromTab, target_clan: S.currentClan,
                    entered_password: S.currentClanPass, record_id: id
                });
    ...
    on('membersUploadBtn', 'click', () => { if (!canEditClan(S.currentClan)) return; const fi = $('membersFileInput'); if (fi) { fi.value = ''; fi.click(); } });
    on('membersFileInput', 'change', async (e) => {
        const file = e.target.files[0]; if (!file) return;
        if (!canEditClan(S.currentClan)) { alert('Нет прав'); return; }
        ...
        try {
            ...
            if (S.isAdmin) {
                const { error } = await supabase.from('clans').update({ members_list: membersText }).eq('id', S.currentClan);
                if (error) throw new Error(error.message);
            } else {
                const { data: resp, error } = await supabase.rpc('clan_admin_action', {
                    action: 'update', target_table: 'clans', target_clan: S.currentClan,
                    entered_password: S.currentClanPass, record_id: S.currentClan, data: { members_list: membersText }
                });
                if (error || resp?.error) throw new Error(resp?.error || error.message);
            }
            if (S.clansCache[S.currentClan]) S.clansCache[S.currentClan].members_list = membersText;
    ...
    on('adminClanImageFile', 'change', async (e) => {
        ...
        try {
            ...
            if (file.type.startsWith('video/')) {
                const reader = new FileReader();
                reader.onload = (ev) => {
                    S.clanLogoData = ev.target.result;
            ...
            if (file.type === 'image/gif' || file.type === 'image/webp') {
                ...
                reader.onload = (ev) => {
                    S.clanLogoData = ev.target.result;
                    $('adminClanImagePreviewImg').src = S.clanLogoData;
                    $('adminClanImagePreview').hidden = false;
            ...
            S.clanLogoData = await compressLogo(file, 256, 0.9);
            $('adminClanImagePreviewImg').src = S.clanLogoData;
            $('adminClanImagePreview').hidden = false;
            $('adminClanImageName').textContent = file.name;
            const inp = $('adminClanImage'); if (inp) inp.value = '';
    ...
    on('newClanImageFile', 'change', async (e) => {
        ...
        try {
            ...
            if (file.type.startsWith('video/') || file.type === 'image/gif' || file.type === 'image/webp') {
                const reader = new FileReader();
                reader.onload = (ev) => {
                    S.newClanLogoData = ev.target.result;
                    if (!file.type.startsWith('video/')) $('newClanImagePreviewImg').src = S.newClanLogoData;
            ...
            S.newClanLogoData = await compressLogo(file, 256, 0.9);
            $('newClanImagePreviewImg').src = S.newClanLogoData;
            $('newClanImagePreview').hidden = false;
...
function bindBuilds() {
    ...
    on('cancelBuildEdit', 'click', () => { $('buildEditModal').hidden = true; S.editingBuild = null; });
    on('saveBuildEdit', 'click', async () => {
        if (!S.editingBuild) return;
        ...
        if (S.editingBuild.type === 'pb' && !rank) { $('buildEditError').textContent = 'Укажите ранг'; return; }
        const data = {
            rank: S.editingBuild.type === 'pb' ? rank : null, ship_name: ship,
        ...
        if (S.isAdmin) {
            const { error } = await supabase.from('builds').update({ clan: clanValue, is_shared: isShared, ...data }).eq('id', S.editingBuild.id);
            if (error) { $('buildEditError').textContent = 'Ошибка: ' + error.message; return; }
        } else {
            const { data: resp, error } = await supabase.rpc('clan_admin_action', {
                action: 'update', target_table: 'builds', target_clan: S.currentClan,
                entered_password: S.currentClanPass, record_id: S.editingBuild.id, data
            });
            if (error || resp?.error) { $('buildEditError').textContent = 'Ошибка: ' + (resp?.error || error.message); return; }
        }
        await logAdminAction(`Изменил билд ${S.editingBuild.type.toUpperCase()}`, ship);
        const type = S.editingBuild.type;
        $('buildEditModal').hidden = true; S.editingBuild = null;
        renderBuilds(type);
    });
    on('cancelBuildDup', 'click', () => { $('buildDupModal').hidden = true; S.duplicatingBuild = null; });
    on('doBuildDup', 'click', async () => {
        if (!S.duplicatingBuild) return;
        const scopeVal = val('buildDupScope');
        const isShared = scopeVal === SHARED;
        const clanValue = isShared ? null : scopeVal;
        const msg = $('buildDupMsg'); msg.style.color = '';
        const item = S.duplicatingBuild;
        if (!canEditClan(S.currentClan)) { msg.textContent = 'Нет прав'; msg.style.color = '#ff7a7a'; return; }
        ...
        setTimeout(() => { $('buildDupModal').hidden = true; S.duplicatingBuild = null; renderBuilds(item.type); }, 700);
    });
}
function bindEvents() {
    on('evAddBtn', 'click', async () => {
        ...
        if (isShared && !S.isOwner) { flashStatusEl(statusEl, 'Общие события создаёт только владелец', '#ff7a7a'); return; }
        if (S.isAdmin) {
            ...
            const { data: resp, error } = await supabase.rpc('clan_admin_action', {
                action: 'insert', target_table: 'events', target_clan: S.currentClan, entered_password: S.currentClanPass,
...
function bindTreasury() {
    on('trAddBtn', 'click', async () => {
        ...
        if (S.isAdmin) {
            const { error } = await supabase.from('treasury').insert({ clan: S.currentClan, type, amount, description: desc });
            if (error) { flashStatusEl(statusEl, 'Ошибка: ' + error.message, '#ff7a7a'); return; }
        } else {
            const { data: resp, error } = await supabase.rpc('clan_admin_action', {
                action: 'insert', target_table: 'treasury', target_clan: S.currentClan, entered_password: S.currentClanPass,
...
function bindShips() {
    ...
    on('ships-level-filters', 'click', e => {
        ...
        chip.classList.add('active'); S.shipsFilterLevel = chip.dataset.level; renderShipsGrid();
    });
    on('ships-type-filters', 'click', e => {
        const chip = e.target.closest('.ships-chip'); if (!chip) return;
        document.querySelectorAll('#ships-type-filters .ships-chip').forEach(c => c.classList.remove('active'));
        chip.classList.add('active'); S.shipsFilterType = chip.dataset.type; renderShipsGrid();
    });
    on('ships-sort', 'change', e => { S.shipsSort = e.target.value; renderShipsGrid(); });
    on('reloadShipsBtn', 'click', loadShips);
}
function bindResources() {
    on('pricingAddBtn', 'click', () => openResourceEditModal(null));
    on('re-cancel', 'click', () => { $('resourceEditModal').hidden = true; S.editingResourceId = null; });
    on('resourceEditModal', 'click', e => { if (e.target.id === 'resourceEditModal') { $('resourceEditModal').hidden = true; S.editingResourceId = null; } });
    ...
    on('re-save', 'click', async () => {
        ...
        const id = (S.editingResourceId || val('re-id').trim().toLowerCase().replace(/[^a-z0-9_-]/g, ''));
        ...
        const image = urlField || $('re-image-url').dataset.dataUrl ||
                      (S.editingResourceId ? S.resourcesCache.find(x => x.id === S.editingResourceId)?.image_url : null);
        ...
        const payload = {
            ...
            sort_order: S.resourcesCache.find(x => x.id === id)?.sort_order ?? (S.resourcesCache.length + 1) * 10,
            updated_at: new Date().toISOString(),
            updated_by: getViewerNick() || 'admin'
        };
        let error;
        if (S.editingResourceId) ({ error } = await supabase.from('resource_prices').update(payload).eq('id', id));
        else ({ error } = await supabase.from('resource_prices').insert(payload));
        if (error) { msg.textContent = 'Ошибка: ' + error.message; msg.style.color = '#ff7a7a'; return; }
        await logAdminAction(S.editingResourceId ? 'Обновил ресурс' : 'Добавил ресурс', name);
        $('resourceEditModal').hidden = true;
        S.editingResourceId = null;
        await loadResourcePrices();
    });
    ...
    on('pricingAuto', 'change', async (e) => {
        ...
        const active = S.tradesCache.filter(t => t.status !== 'done' && t.type === 'sell' && t.category === 'resource');
        ...
        let updated = 0;
        for (const r of S.resourcesCache) {
...
function bindRecipes() {
    ...
    on('disc-add', 'click', async () => {
        ...
        const { error } = await supabase.from('discounts').insert({
            ...
            sort_order: (S.discountsCache.length + 1) * 10
        });
        if (error) { msg.textContent = 'Ошибка: ' + error.message; msg.style.color = '#ff7a7a'; return; }
...
function bindMap() {
    ...
    document.querySelectorAll('.map-view-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            S.currentMapView = btn.dataset.view;
            renderMapPreview();
            const fs = $('mapFullscreen');
            if (fs && !fs.hidden) {
                const urls = getMapUrls();
                const img = $('map-fs-img');
                if (img) img.src = S.currentMapView === 'clean' ? urls.clean : urls.detailed;
                resetMapFs();
            }
        });
    });
    on('map-fs-close', 'click', closeMapFullscreen);
    on('map-fs-zoom-in', 'click', () => setMapFsZoom(S.mapFsZoom + 0.25));
    on('map-fs-zoom-out', 'click', () => setMapFsZoom(S.mapFsZoom - 0.25));
    on('map-fs-reset', 'click', resetMapFs);
    on('map-fs-stage', 'wheel', e => { e.preventDefault(); setMapFsZoom(S.mapFsZoom + (e.deltaY < 0 ? 0.15 : -0.15)); }, { passive: false });
    on('map-fs-stage', 'mousedown', e => {
        if (S.mapFsZoom <= 1) return;
        S.mapFsDragging = true;
        S.mapFsDragStart = { x: e.clientX - S.mapFsX, y: e.clientY - S.mapFsY };
        $('map-fs-stage').classList.add('dragging');
    });
    document.addEventListener('mousemove', e => {
        if (!S.mapFsDragging) return;
        S.mapFsX = e.clientX - S.mapFsDragStart.x;
        S.mapFsY = e.clientY - S.mapFsDragStart.y;
        applyMapFsTransform();
    });
    document.addEventListener('mouseup', () => {
        if (!S.mapFsDragging) return;
        S.mapFsDragging = false;
        $('map-fs-stage')?.classList.remove('dragging');
    });
    on('map-fs-stage', 'mouseleave', () => { S.mapFsDragging = false; });
    ...
    on('map-save', 'click', async () => {
        ...
        const detailedUrl = S.mapDetailedData || val('map-detailed-url').trim() || null;
        const cleanUrl = S.mapCleanData || val('map-clean-url').trim() || null;
...
function bindFactions() {
    ...
    on('pe-save', 'click', async () => {
        ...
        if (!name) { msg.textContent = 'Укажи название'; msg.style.color = '#ff7a7a'; return; }
        if (S.portsCache.some(p => p.name === name && String(p.id) !== String(id))) { msg.textContent = 'Порт с таким именем уже есть'; msg.style.color = '#ff7a7a'; return; }
    ...
    on('fe-save', 'click', async () => {
        ...
        await loadFactions(); await loadPorts();
    });
    on('port-add', 'click', async () => {
        ...
        if (!name) { msg.textContent = 'Укажи название'; msg.style.color = '#ff7a7a'; return; }
        if (S.portsCache.some(p => p.name === name)) { msg.textContent = 'Такой порт уже есть'; msg.style.color = '#ff7a7a'; return; }
        const { error } = await supabase.from('ports').insert({ name, type, faction_id: factionId, region, can_be_captured: capturable, note, sort_order: (S.portsCache.length + 1) * 10 });
        if (error) { msg.textContent = 'Ошибка: ' + error.message; msg.style.color = '#ff7a7a'; return; }
...
function bindTrades() {
    document.querySelectorAll('.tm-type-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            btn.classList.add('active'); S.tradeFormType = btn.dataset.type;
        });
    });
    ...
    on('tm-only-ships', 'change', e => { S.tradeOnlyShips = e.target.checked; renderTradeListings(); });
    ...
    on('tm-sort', 'change', e => { S.tradeSort = e.target.value; renderTradeListings(); });
    ...
    on('tm-submit', 'click', async () => {
        ...
        const { error } = await supabase.from('trades').insert({
            clan, type: S.tradeFormType, category, name, price, qty,
    ...
    on('tradeEditCancel', 'click', () => { $('tradeEditModal').hidden = true; S.editingTradeId = null; });
    on('tradeEditModal', 'click', e => { if (e.target.id === 'tradeEditModal') { $('tradeEditModal').hidden = true; S.editingTradeId = null; } });
    on('tradeEditSave', 'click', async () => {
        if (!S.editingTradeId) return;
        ...
        const { error } = await supabase.from('trades').update({ name, price, qty, port: port || null, note: note || null }).eq('id', S.editingTradeId);
        if (error) { err.textContent = 'Ошибка: ' + error.message; return; }
        await logAdminAction('Изменил торговую заявку', name);
        $('tradeEditModal').hidden = true; S.editingTradeId = null; renderTrades();
    });
    ...
    on('doAcceptTrade', 'click', async () => {
        ...
        if (nick.toLowerCase() === (S.acceptingTrade.nickname || '').toLowerCase()) { err.textContent = 'Нельзя принять свою заявку'; return; }
        $('doAcceptTrade').disabled = true;
        const { error } = await supabase.from('trades').update({ accepted_by: nick, accepted_at: new Date().toISOString() }).eq('id', S.acceptingTrade.id);
        $('doAcceptTrade').disabled = false;
        if (error) { err.textContent = 'Ошибка: ' + error.message; return; }
        localStorage.setItem(VIEWER_NICK_KEY, nick);
        sendHeartbeat();
        await createNotification(S.acceptingTrade.nickname, 'trade', '🤝 Вашу заявку приняли', `${nick} принял заявку «${S.acceptingTrade.name}». Подтвердите сделку.`, null);
...
function bindContent() {
    on('faqAddBtn', 'click', async () => {
        ...
        const { error } = await supabase.from('faq').insert({ question: q, answer: a, sort_order: S.faqCache.length + 1 });
        if (error) return alert(error.message);
    ...
    on('partnerLogoFile', 'change', async e => {
        const file = e.target.files[0]; if (!file) return;
        try {
            S.partnerLogoData = await compressLogo(file);
            $('partnerLogoImg').src = S.partnerLogoData;
    ...
    on('partnerLogoClear', 'click', () => {
        S.partnerLogoData = null; $('partnerLogoFile').value = '';
    ...
    on('partnerAddBtn', 'click', async () => {
        ...
        const { error } = await supabase.from('partners').insert({ name, url, logo_url: S.partnerLogoData || null, description: desc || null, sort_order: S.partnersCache.length });
        if (error) { flashStatusEl(statusEl, 'Ошибка: ' + error.message, '#ff7a7a'); return; }
    ...
    on('cancelTacEdit', 'click', () => { $('tacticsEditModal').hidden = true; S.editingTactic = null; });
    on('saveTacEdit', 'click', async () => {
        if (!S.editingTactic) return;
        ...
        const { error } = await supabase.from('tactics').update({ title, icon: icon || null, content, sort_order: order, updated_at: new Date().toISOString() }).eq('id', S.editingTactic.id);
        if (error) { msg.textContent = 'Ошибка: ' + error.message; msg.style.color = '#ff7a7a'; return; }
        $('tacticsEditModal').hidden = true; S.editingTactic = null; await loadTactics();
    });
...
function bindLeader() {
    ...
    on('leaderOfferAllianceBtn', 'click', () => {
        ...
        const myAlliance = S.clansCache[myClanId]?.alliance_id;
        sel.innerHTML = '';
        let count = 0;
        Object.values(S.clansCache).forEach(c => {
    ...
    on('leaderAlliancePickerSend', 'click', async () => {
        ...
        if (error) { err.textContent = 'Ошибка: ' + error.message; return; }
        await createNotification(toClan, 'system', 'Заявка на союз', `Гильдия «${S.clansCache[myClanId]?.name || myClanId}» предлагает союз`, null);
...
function bindAdmin() {
    ...
    on('saveAdminSettings', 'click', async () => {
        ...
        if (error) { msg.textContent = 'Ошибка: ' + error.message; msg.style.color = '#ff7a7a'; return; }
        Object.assign(S.clansCache[cid], payload);
        msg.textContent = '✔ Сохранено'; msg.style.color = '#6ee7a7';
        $('adminNewPass').value = ''; $('adminNewAdminPass').value = '';
        $('adminCurrentPass').value = S.clansCache[cid].password || '—';
        $('adminCurrentAdminPass').value = S.clansCache[cid].admin_password || '—';
        if (S.currentClan === cid) { renderMembers(); renderAdmins(); }
        renderHomeCards(); renderContacts(); renderAlliancesAdmin(); updateAllianceBar();
    });
    on('deleteClanBtn', 'click', async () => {
        const cid = val('adminClanSelect'); if (!cid) return;
        const clan = S.clansCache[cid]; if (!clan) return alert('Гильдия не найдена');
        ...
        try {
            ...
            if (error) throw error;
            delete S.clansCache[cid];
            if (S.currentClan === cid) { S.currentClan = null; [LAST_CLAN_KEY, UNLOCK_KEY, MY_CLAN_KEY].forEach(k => localStorage.removeItem(k)); }
    ...
    on('saveNewClan', 'click', async () => {
        ...
        if (S.clansCache[id]) { msg.textContent = 'ID занят'; msg.style.color = '#ff7a7a'; return; }
        const finalImage = S.newClanLogoData || val('newClanImage').trim() || null;
        const payload = {
    ...
    on('saveSiteSettings', 'click', async () => {
        ...
        if (error) { msg.textContent = 'Ошибка: ' + error.message; msg.style.color = '#ff7a7a'; return; }
        S.settingsCache = Object.assign({ id: 'main' }, S.settingsCache || {}, payload);
        msg.textContent = '✔ Сохранено'; msg.style.color = '#6ee7a7';
    });
...
async function renderApplications() {
    ...
    data.forEach(app => {
        ...
        const targetClanName = app.target_clan && S.clansCache[app.target_clan] ? S.clansCache[app.target_clan].name : (app.target_clan || '—');
        card.innerHTML = `
            <div class="application-head">
...
function bindAdminClanEvents() {
    // Владелец принимает участие в общих событиях через "Панель главы" — уже покрыто bindLeader
}

/* ════════════════════════════════════════════════════════════════════════
   MODULE 30 — BOOTSTRAP
   Точка входа. Порядок важен: сначала все bindings, потом загрузка данных.
   ════════════════════════════════════════════════════════════════════════ */
(async () => {
    ...
    fillScFactionSelect();
    fillScCities();
    S.currentMapView = (S.mapSettings?.default_view) || 'detailed';
    ...
    // Восстановление последней гильдии
    const lastClan = localStorage.getItem(LAST_CLAN_KEY);
    if (lastClan && isUnlocked() && S.clansCache[lastClan]) {
    ...
    // Обработка #build=hash
    setTimeout(() => {
        const m = location.hash.match(/^#build=([a-f0-9-]+)$/i);
        if (!m) return;
        (async () => {
            const { data } = await supabase.from('builds').select('*').eq('id', m[1]).single();
            if (!data) return;
    ...
    window.addEventListener('hashchange', () => { if (location.hash.startsWith('#build=')) location.reload(); });

    // Периодические обновления
    setInterval(() => {
        const onlineSection = document.querySelector('.admin-section[data-apanel="online"]');
        if (onlineSection && onlineSection.classList.contains('active') && S.isAdmin) renderAdminOnlineList();
        const clanOnlineSection = $('section-online');
        if (clanOnlineSection && clanOnlineSection.classList.contains('active') && canEditClan(S.currentClan)) {
